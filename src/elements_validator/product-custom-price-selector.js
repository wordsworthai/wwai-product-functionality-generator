import { PRICE_LABEL_ATTR, PRICE_DATA_ATTRS, PRICE_WRAPPER_ATTRS } from '../constants/elements/price-selector-constants.js';
...
function validateAllElementsHaveLabels(elements, context = '<product-custom-price-selector>') {
    const messages = [];
...
function validateLabelEntry(label, { beforeCount, afterCount, discountCount }, containerEl) {
    const messages = [];
    let hasError = false;
    let hasWarning = false;
    ...
    if (beforeCount > 1 || afterCount > 1) {
        hasError = true;
        messages.push(
        `Label "${label}" has multiple elements: ${[
...
export function getValidationStatusForSelector(containerEl, context = '<product-custom-price-selector>') {
    ...
    // 🔍 Check that every price-related element has a label
    ...
    // 🛑 Stop early if no elements have a valid label at all
    ...
    // 🧠 Decide final status
    const finalStatus = hasLabelErrors
      ? 'error'
      : downstreamStatus;
...
export function validateCustomPriceSelector(selectorElement, printToLogs = false) {  
    const { status, messages } = getValidationStatusForSelector(selectorElement, '<product-custom-price-selector>');
